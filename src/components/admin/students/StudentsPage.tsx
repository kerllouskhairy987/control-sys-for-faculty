...
export default function StudentsPage() {
    ...
    const handleConfirmDelete = () => {
        toast.success(`${deleteConfirmation.student?.fullName} deleted (static mode)`);
        setDeleteConfirmation({ isOpen: false, student: null });
    };

    const handleModalSubmit = async (formData: StudentFormData) => {
        if (selectedStudent) {
            toast.success('Student updated (static mode)');
        } else {
            toast.success('Student created (static mode)');
        }
